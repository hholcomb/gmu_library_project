...
public class InMemoryAuthorDao implements AuthorDao {
    ...
    private static final Logger logger = Logger.getLogger(InMemoryAuthorDao.class.getName());

    //  the following are the arrays for the author data

    private Integer [] author_id =  { 1, 2, 3, 4, 5
                            };



    private String [] author_name = { "J. R. R. Tolkien",
                              "Ayn Rand",
                              "Orson Scott Card",
                              "William Shakespeare",
                              "Neal Stephenson"
    ...
    private String [] author_dob = { "1892-01-03" ,
                             "1905-02-02",
                             "1951-08-24",
                             "1564-04-26",
                             "1959-10-31"
    ...
    private String [] author_nationality = { "British" ,
                                     "United States",
                                     "United States",
                                     "British",
                                     "United States"
    ...
    private String [] author_biography = { "John Ronald Reuel Tolkien, CBE (tol-keen; 3 January 1892 – 2 September 1973) was an English writer, poet, philologist, and university professor, best known as the author of the classic high fantasy works The Hobbit, The Lord of the Rings, and The Silmarillion." ,
    ...
    private String [] author_image_url = { "http://upload.wikimedia.org/wikipedia/commons/b/b4/Tolkien_1916.jpg",
...
public interface AuthorDao {
    ...
    //      create the collection to return to the caller
        ...
        //returnCollection = Collections.EMPTY_SET;
         HashSet<Author> returnCollection =  new HashSet<>();

    //      determine the number of elements in the authors information arrays
        array_size = author_nationality.length;

        logger.info("getAllAuthors - will now add each author to the collection");
        logger.info( "number of entries = " + array_size );

        if ( array_size < 1)
        {
            //      this is an error as there should always be entries
            return null;
        }
        else
        {
          ...
          {
            //logger.info( " getAllAuthors - ready to call getAuthor" );

            entryToAdd = getAuthor( index );
            //logger.info( " getAllAuthors - have retrieved an author");

            try {
              returnCollection.add( entryToAdd );
              logger.info( "getAllAuthors - have added an author to the collection");
            } catch ( Exception e ) {
                e.printStackTrace();
            }
          }
